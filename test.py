...
def calculate_total_days(year_start, month_start, day_start, year_end, month_end, day_end):
    ...
    total_days = ((end_datetime - start_datetime).days + 12)
    return total_days

# 示例使用：
year_start = input("请输入起始日期的年份：")   #user_year
month_start = input("请输入起始日期的月份：")  # 1
day_start = input("请输入起始日期的日期：")    # 1
year_end = input("请输入结束日期的年份：")     #user_year
month_end = input("请输入结束日期的月份：")    #user_month
day_end = input("请输入结束日期的日期：")      #user_day
...
while user_year <= 1900 or 1 <= user_month >= 12 or 1 <= user_day >= 31 or 0 <= user_time >= 23:
    print("Invalid input(s)! Please enter again.\n")
    user_year = int(input("Please enter your Birth Year (Year 1900 and above): "))
    user_month = int(input("Please enter your birth month (Month 1-12): "))
    user_day = int(input("Please enter your birth date (Day 1-31): "))
    user_time = int(input("Please enter your birth time (0-23): "))
...
#Birth Month Part    
    if 1 <= user_month <= 12 :
        ...
        if lunar_month != 0:
            Mnum_TG = lunar_month - 1 
            月天干 = TG_list[Mnum_TG]
        else:
            月天干 = TG_list[-1]

        Mnum_DZ = user_month - 1
